...
extension XCTApplicationTester {
    public func login(user: Player) throws -> TokenAuth {
        ...
        return try response.content.decode(TokenAuth.self)
    }
...
public func test(_ method: HTTPMethod,
        ...
        if (loggedInRequest || loggedInUser != nil) {
            let userToLogin: Player
            if let user = loggedInUser {
                userToLogin = user
            } else {
                userToLogin = Player(firstName: "Admin",
                                   lastName: "Admin",
                                   username: "admin",
                                   password: "password",
